...
function renderPresets(){
  ...
  PRESETS.forEach(url=>{
    const d=document.createElement('div'); d.style.width='44px'; d.style.height='44px'; d.style.borderRadius='50%'; d.style.overflow='hidden'; d.style.cursor='pointer';
    d.innerHTML=`<img src="${url}" style="width:44px;height:44px;display:block" />`;
    d.onclick=()=>{ chosenAvatar=url; root.querySelectorAll('div').forEach(x=>x.style.outline=''); d.style.outline='3px solid rgba(59,130,246,0.18)'; };
    root.appendChild(d);
  });
}
renderPresets();
document.getElementById('uploadAvatar')?.addEventListener('change', async (e)=>{
  ...
  try{
    ...
    await supabase.from('profiles').upsert({ id: currentUser.id, email: currentUser.email, avatar_url: publicURL }, { onConflict: ['id'] });
...
async function loadProfile(){
  ...
  try{
    ...
    if(!data){
      await supabase.from('profiles').insert({ id: currentUser.id, email: currentUser.email, avatar_url: chosenAvatar, streak:0 });
      profile = { id: currentUser.id, email: currentUser.email, avatar_url: chosenAvatar, streak:0, is_admin:false };
    } else profile = data;
...
function onSignedIn(){
  ...
  renderUserInfo();
  fetchLeaderboard();
  supabase.from('profiles').upsert({ id: profile.id, email: profile.email, avatar_url: profile.avatar_url || chosenAvatar }, { onConflict: ['id'] }).then(()=>{});
...
btnSignUp?.addEventListener('click', async ()=>{
  ...
  try{
    const { data, error } = await supabase.auth.signUp({ email, password: pw, options: { data: { avatar: chosenAvatar } } });
    if(error){ showAuth(error.message, true); return; }
    // create profile row
    await supabase.from('profiles').upsert({ id: data.user.id, email, avatar_url: chosenAvatar, streak:0 }, { onConflict: ['id'] });
...
function renderBoard(){
  const grid=document.getElementById('cardGrid'); if(!grid) return; grid.innerHTML=''; document.getElementById('foundCount').textContent=setsFound;
  cards.forEach((c,i)=>{ const btn=document.createElement('div'); btn.className='cardTile'; btn.dataset.index=i; btn.innerHTML=`<div style="font-weight:700">${numbers[c.num]}</div><div>${shapes[c.shape]}</div><div class="muted">${colors[c.color]} • ${fills[c.fill]}</div>`; btn.onclick=()=>{ btn.classList.add('pop'); setTimeout(()=>btn.classList.remove('pop'),220); selectCard(i); }; grid.appendChild(btn); });
  document.getElementById('adminTools').style.display = (profile?.is_admin)?'block':'none';
  renderUserInfo();
}
function selectCard(i){ const el=document.querySelector(`[data-index="${i}"]`); if(!el) return; if(selected.includes(i)){ selected=selected.filter(x=>x!==i); el.classList.remove('selected'); return;} if(selected.length<3){ selected.push(i); el.classList.add('selected'); } if(selected.length===3) checkSelected(); }
function checkSelected(){ const chosen=selected.map(i=>cards[i]); const els=selected.map(i=>document.querySelector(`[data-index="${i}"]`)).filter(Boolean); if(chosen.length!==3){ selected=[]; return; } if(isSet(...chosen)){ els.forEach(e=>e.classList.add('correct')); setsFound++; const fc=document.getElementById('foundCount'); if(fc) fc.textContent=setsFound; if(setsFound===6){ stopTimer(); onWin(); } } else els.forEach(e=>e.classList.add('wrong')); setTimeout(()=>els.forEach(e=>e.classList.remove('selected','wrong','correct')),700); selected=[]; }

/* ---------- Confetti + sound ---------- */
function fireConfetti(){
  const canvas=document.getElementById('confetti'); if(!canvas) return; const ctx=canvas.getContext('2d'); canvas.width=canvas.clientWidth; canvas.height=canvas.clientHeight;
...
document.getElementById('overlayShare').addEventListener('click', async ()=>{
  ...
  const canvas = document.getElementById('shareCanvas'); canvas.width=800; canvas.height=420; const ctx=canvas.getContext('2d');
  ctx.fillStyle = getComputedStyle(document.documentElement).getPropertyValue('--card').trim() || '#0f1724'; ctx.fillRect(0,0,canvas.width,canvas.height);
  const img = new Image(); img.crossOrigin='anonymous'; img.src = profile?.avatar_url || chosenAvatar;
  img.onload = async ()=>{
...
async function updateStreak(username){
  ...
  try{
    ...
    const last = data.last_played; const diff = Math.floor((new Date(today)-new Date(last)) / (1000*60*60*24));
    let newStreak = 1;
    if(diff===1) newStreak = data.streak_count + 1; else if(diff===0) newStreak = data.streak_count; else newStreak =1;
...
/* fetchLeaderboard with debounce */
let lbTimeout = null;
async function fetchLeaderboard(){
  if(lbTimeout) clearTimeout(lbTimeout);
  return new Promise(resolve=>{
    lbTimeout = setTimeout(async ()=>{
      try{
