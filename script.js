...
/* initialize supabase and also attach to window for quick checks */
window.supabase = supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
const sb = window.supabase; // shorthand
...
const presetAvatarsRoot = document.getElementById('presetAvatars');
...
function renderPresets(){
  if(!presetAvatarsRoot) return;
  presetAvatarsRoot.innerHTML = '';
  PRESETS.forEach(url=>{
    ...
    d.onclick = ()=>{ chosenAvatar = url; presetAvatarsRoot.querySelectorAll('div').forEach(x=>x.style.outline=''); d.style.outline='3px solid rgba(59,130,246,0.18)'; };
    presetAvatarsRoot.appendChild(d);
  });
}
renderPresets();
...
document.getElementById('uploadAvatar')?.addEventListener('change', async (e)=>{
  ...
  try{
    ...
    const publicRes = sb.storage.from('avatars').getPublicUrl(path);
    const publicURL = publicRes?.data?.publicUrl || publicRes?.publicUrl || null;
    if(!publicURL) throw new Error('No public URL');
    await sb.from('profiles').upsert({ id: currentUser.id, email: currentUser.email, avatar_url: publicURL }, { onConflict: ['id'] });
    profile = { ...(profile||{}), avatar_url: publicURL }; chosenAvatar = publicURL; renderUserInfo();
    alert('Avatar uploaded');
  }catch(err){ console.warn('upload failed',err); alert('Avatar upload failed (check console)'); }
});
...
btnSignUp?.addEventListener('click', async ()=>{
  ...
  try{
    const { data, error } = await sb.auth.signUp({ email, password: pw, options: { data: { avatar: chosenAvatar } } });
    if(error){ showAuth(error.message, true); return; }
    // create profile row (id === auth.users.id)
    await sb.from('profiles').upsert({ id: data.user.id, email, avatar_url: chosenAvatar, streak:0 }, { onConflict: ['id'] });
    showAuth('Account created — confirm email then sign in (or disable confirmations)');
...
async function loadProfile(){
  ...
  try{
    ...
    if(!data){
      await sb.from('profiles').insert({ id: currentUser.id, email: currentUser.email, avatar_url: chosenAvatar, streak:0 });
      profile = { id: currentUser.id, email: currentUser.email, avatar_url: chosenAvatar, streak: 0, is_admin: false, achievements: [] };
    } else profile = data;
...
function renderUserInfo(){
  ...
  // badges
  const badges = [];
  if(profile?.streak >= 3) badges.push('<span class="badge">🔥 3-day streak</span>');
  if(profile?.streak >= 7) badges.push('<span class="badge">🏆 7-day streak</span>');
  ui.innerHTML = `${avatar}<strong>${email}</strong> ${badges.join('')}`;
}
...
/* ---------- Confetti + sound ---------- */
function fireConfetti(){ const canvas=document.getElementById('confetti'); if(!canvas) return; const ctx=canvas.getContext('2d'); canvas.width=canvas.clientWidth; canvas.height=canvas.clientHeight; const pieces=[]; const colorsArr=['#ff4757','#ffa502','#2ed573','#1e90ff','#9b59b6']; for(let i=0;i<110;i++) pieces.push({x:Math.random()*canvas.width,y:Math.random()*-canvas.height,vx:(Math.random()-0.5)*6,vy:Math.random()*6+2,size:Math.random()*8+4,color:colorsArr[Math.floor(Math.random()*colorsArr.length)]}); let t=0; function frame(){ ctx.clearRect(0,0,canvas.width,canvas.height); pieces.forEach(p=>{ p.x+=p.vx; p.y+=p.vy; ctx.fillStyle=p.color; ctx.fillRect(p.x,p.y,p.size,p.size*0.6); }); t++; if(t<180) requestAnimationFrame(frame); else ctx.clearRect(0,0,canvas.width,canvas.height); } requestAnimationFrame(frame); playConfettiSound(); }
...
async function onWin(){
  ...
  fireConfetti();
  const overlaySave = document.getElementById('overlaySave'); if(overlaySave){ overlaySave.disabled = false; overlaySave.dataset.seconds = seconds; }
  // achievements: fast finisher (example)
  if(seconds <= 30){
    await awardAchievement('Speedster', `Completed in ${seconds}s`);
  }
}
...
document.getElementById('overlayShare').addEventListener('click', async ()=>{
  try{
    ...
    const canvas = document.getElementById('shareCanvas'); canvas.width = 800; canvas.height = 420;
    ...
    img.onload = async ()=>{
      ...
      ctx.fillText(`Time: ${seconds}s`, 180, 170);
      ...
      canvas.toBlob(async (blob)=>{
        if(navigator.canShare && navigator.canShare({ files: [new File([blob], 'dailyset.png', { type: 'image/png' })] })){
          try{ await navigator.share({ files: [new File([blob], 'dailyset.png', { type: 'image/png' })], title: 'Daily Set Score', text: `I solved Daily Set in ${seconds}s!` }); return; }catch(e){ console.warn('share failed', e); }
        }
...
async function submitScore(username, secondsTaken=0){
  ...
  try{
    await sb.from('leaderboard').insert([{ username, avatar_url: profile?.avatar_url || null, score:6, sets_found:6, seconds_taken: secondsTaken, game_date: today }]);
...
async function fetchLeaderboard(){
  try{
    const { data } = await sb.from('leaderboard').select('*').order('seconds_taken',{ascending:true}).order('inserted_at',{ascending:true}).limit(50);
...
function animateLeaderboard(rows){
  ...
  rows.forEach((r,i)=>{ const li = document.createElement('li'); li.className = 'lb-item'; li.innerHTML = `${r.avatar_url?`<img src="${r.avatar_url}" class="avatarSmall">` : ''}<strong>${r.username}</strong> — <strong>${r.seconds_taken}s</strong> <span class="muted">(${new Date(r.inserted_at).toLocaleDateString()})</span>`; ol.appendChild(li); setTimeout(()=> li.classList.add('visible'), 50 + i*70); });
}

/* realtime subscription for leaderboard inserts */
function subscribeLeaderboardRealtime(){
  try{
    const channel = sb.channel('public:leaderboard')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'leaderboard' }, payload => {
        // prepend new item and animate
        const r = payload.new;
        const ol = document.getElementById('leaderboardList'); if(!ol) return;
        const li = document.createElement('li'); li.className = 'lb-item';
        li.innerHTML = `${r.avatar_url?`<img src="${r.avatar_url}" class="avatarSmall">` : ''}<strong>${r.username}</strong> — <strong>${r.seconds_taken}s</strong> <span class="muted">(now)</span>`;
...
async function updateStreak(username){
  const today = new Date().toISOString().split('T')[0];
  try{
    const { data } = await sb.from('streaks').select('*').eq('username', username).maybeSingle();
    if(!data){ await sb.from('streaks').insert([{ username, last_played: today, streak_count: 1 }]); await sb.from('profiles').update({ streak:1 }).eq('email', username); return 1; }
    ...
    await sb.from('profiles').update({ streak: newStreak }).eq('email', username);
    // award streak badges
    if(newStreak === 3) await awardAchievement('3-day streak', 'Keep it going!');
    if(newStreak === 7) await awardAchievement('7-day streak', 'Legendary streak!');
    return newStreak;
  }catch(e){ console.warn('updateStreak', e); return null; }
}

async function awardAchievement(key, detail){
  try{
    // store simple achievement in profiles. This uses a JSON array in profiles.achievements (optional)
    const ach = { key, detail, at: new Date().toISOString() };
    const existing = profile?.achievements || [];
    const found = existing.find(a=>a.key===key);
    if(found) return;
    const newArr = [...existing, ach];
    await sb.from('profiles').update({ achievements: newArr }).eq('id', profile.id);
    profile.achievements = newArr;
    // small UI hint
    alert(`Achievement unlocked: ${key}`);
  }catch(e){ console.warn('awardAchievement', e); }
}

/* ---------- Admin toggle (Ctrl+Shift+A) ---------- */
document.addEventListener('keydown', (e)=>{ if(e.ctrlKey && e.shiftKey && e.key.toLowerCase()==='a'){ if(!profile?.is_admin){ alert('Admin only'); return; } adminMode = !adminMode; const at = document.getElementById('adminTools'); if(at) at.style.display = adminMode ? 'block' : 'none'; if(adminMode){ renderAdminUserList(); renderAdminLeaderboard(); } } });

/* admin helpers */
async function renderAdminLeaderboard(){ const container=document.getElementById('adminLeaderboard'); if(!container) return; container.innerHTML='Loading...'; try{ const { data } = await sb.from('leaderboard').select('*').order('inserted_at',{ascending:false}).limit(200); container.innerHTML=''; data.forEach(row=>{ const d=document.createElement('div'); d.style.marginBottom='6px'; d.innerHTML = `${row.avatar_url?`<img src="${row.avatar_url}" class="avatarSmall">` : ''}<strong>${row.username}</strong> — ${row.seconds_taken}s <button onclick="adminDeleteScore(${row.id})" style="margin-left:8px;background:#e11d48">Delete</button>`; container.appendChild(d); }); }catch(e){ console.warn(e); container.innerHTML='<div class="muted">Failed</div>'; } }
window.adminDeleteScore = async function(id){ if(!confirm('Delete?')) return; try{ await sb.from('leaderboard').delete().eq('id', id); renderAdminLeaderboard(); fetchLeaderboard(); }catch(e){ console.warn(e); alert('Delete failed'); } }
async function renderAdminUserList(){ if(!profile?.is_admin) return; const container=document.getElementById('adminUserList'); if(!container) return; container.innerHTML='Loading users...'; try{ const { data } = await sb.from('profiles').select('*').limit(200); container.innerHTML=''; data.forEach(u=>{ const row=document.createElement('div'); row.style.marginBottom='6px'; row.innerHTML = `${u.avatar_url ? `<img src="${u.avatar_url}" class="avatarSmall">` : ''}<strong>${u.email}</strong> Streak: ${u.streak || 0} <button style="margin-left:8px" onclick="adminResetPw('${u.email}')">Send reset</button>`; container.appendChild(row); }); }catch(e){ console.warn(e); container.innerHTML='<div class="muted">Failed</div>'; } }
window.adminResetPw = function(email){ if(!confirm(`Send password reset to ${email}?`)) return; sb.auth.resetPasswordForEmail(email).then(()=> alert('Email sent')).catch(e=>{ console.warn(e); alert('Failed'); }); }

/* ---------- UI transitions & wiring ---------- */
function onSignedIn(){
  ...
  sb.from('profiles').upsert({ id: profile.id, email: profile.email, avatar_url: profile.avatar_url || chosenAvatar }, { onConflict: ['id'] }).then(()=>{});
...
document.getElementById('darkToggle')?.addEventListener('click', ()=>{
  ...
  document.getElementById('darkToggle').textContent = next === 'dark' ? 'Dark' : 'Light';
});
...
/* ---------- realtime subscription starter for leaderboard ---------- */
subscribeLeaderboardRealtime();
